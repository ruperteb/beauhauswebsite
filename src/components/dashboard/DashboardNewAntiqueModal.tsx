...
export const DashboardNewAntiqueModal: React.FunctionComponent<Props> = ({ }) => {
    ...
    const [createItem, { data, loading, error }] = useMutation<Mutation, MutationCreateItemArgs>(
        CREATE_ITEM,
        {
            onCompleted: (result) => {
                if (result) {
    ...
    return (
        ...
        <Modal
            onClose={() => dispatch(dashboardSlice.actions.setShowNewAntiqueModal(false))}
            open={showNewAntiqueModal}
            size='small'
        >
            <StyledModalHeader>
                ...
                <StyledModalHeaderIcon link name='close' onClick={() => dispatch(dashboardSlice.actions.setShowNewAntiqueModal(false))} />
            </StyledModalHeader>
            <Modal.Content>
                <Formik
                    initialValues={{
...
export default DashboardNewAntiqueModal